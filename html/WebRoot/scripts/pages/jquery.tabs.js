(function($) {
	...
	function setSize(container) {
		...
		var tabHeight = $('>div.tabs-header ul.tabs', container)
				.outerHeight(true);
	...
	function createTab(container, options) {
		...
		if (options.icon) {
			...
			tab_span.after($('<span/>').addClass('tabs-icon')
		...
		// 写Cookie
		setCookie(options.title, options.navigation);
	...
	function addTab(container, options) {
		...
		if (options.selected) {
			$('.tabs-header .tabs-wrap .tabs li', container)
	...
	function closeTab(container, title, type) {
		
		if (type == 'ture') {
			// modify by xujiakun 2009-12-17
			// 选择下一个可选tab
			selectNextTab(title);
		}
	...
	$.fn.tabs = function(options, param) {
			...
			switch (options) {
				...
				case 'close' :
					return this.each(function() {
								...
								closeTab(this, param.title, param.type);
							});
				case 'select' :
					return this.each(function() {
								// modify by xujiakun 2009-12-07
								selectTab(this, param.title);
							});
			}
		}
		...
		return this.each(function() {
					...
					$('.tabs-scroller-left', header).bind('click.tabs',
							onClickScrollLeft);
					$('.tabs-scroller-right', header).bind('click.tabs',
							onClickScrollRight);
					...
					function onCloseTab() {
						...
						var tabAttr = $.data(elem, 'tabs.tab');
						closeTab(container, tabAttr.title, 'ture');
					}
					...
					function onClickTab() {
						...
						$('>div.tabs-panels>div', container).css('display',
								'none');
						...
						if (left < 0 || right > wrap.innerWidth()) {
							var pos = Math.min(leftPos
											- (wrap.width() - $(this).width())
											/ 2, getMaxScrollWidth(container));
							wrap.animate({
										scrollLeft : pos
									}, opts.scrollDuration);
						}
						...
						// modify by xujiakun 2009-11-25
						if (tabAttr.navigation != "") {
							var form = window.document.forms[0];
							form.action = tabAttr.navigation;
							form.submit();
						}

						// modify by xujiakun 2009-12-17
						// 保存上一次被点击tab
						setLastClickTab(tabAttr.title);
						...
						// Google Analytics
						_gaq.push( [ '_trackPageview', '/IMS/' + tabAttr.navigation ]);
					}
