...
function openCenterWindow(address,widthx,hightx){
			...
			openWindow(address,windowname,'Height=' + windowhight + ',Left=' + left + ',Top='+ top +',Width=' + windowwidth + ',scrollbars=1,status=no,toolbar=no,menubar=no,location=no');
}
...
function SelectAll_Checkbox(checkbox){
	...
	for (i=0;i<checkbox.length;i++){
		if (checkbox[i].checked == false && checkbox[i].disabled == false){  // “checkbox[i].disabled == false”modify by 小艳
...
function isNum_text(itemname,promptmsg){
	...
	if (itemx != null){
		if (itemx.value != undefined && lTrim(itemx.value) == ""){
			...
			if(fucCheckNUM(itemx.value)==0) return promptmsg + "必须填写数字\n";
			else return "";
		}
	}else{
		return "表单项:" + itemname + "未定义,无法判断用户输入的数据!";
...
function isFloatNum_text(itemname,promptmsg){
	...
	if (itemx != null){
		if (itemx.value != undefined && lTrim(itemx.value) == ""){
			...
			if(fucCheckFloatNUM(itemx.value)==0) return promptmsg + "必须填写数字\n";
			else return "";
		}
	}else{
		return "表单项:" + itemname + "未定义,无法判断用户输入的数据!";
...
//*******************************************************
function single_checkbox(formId,promptmsg){
	var itemx = $('#'+formId+' :checkbox:not(#selectAllChkBox)');
	if (itemx != null)
	{
		var checked = 0;
...
function selectedValQuery(formId) {
	var i = 0;
	$('#'+formId+' :checkbox:not(#selectAllChkBox)').each(function() {
...
//检查表单内容单选框复选框是否为空
//*******************************************************
function isnull_radiobutton(formId,promptmsg){
	var itemx = selectedValQuery(formId);
	if (formId != null){
...
function calCtrlPopup1(inputFieldName) {
	showCalendar(inputFieldName, 'y-mm-dd');
}
...
function createShadeDiv(){
	...
	var sb = [];
	sb.push('<iframe scrolling="no" width="100%" height="100%" frameborder="0"');
	sb.push('id="shadeIframe" name="shadeIframe" ');
	sb.push('style="z-index:99;position:absolute; top:0px;left:0px;filter:alpha(opacity=0);"');
	sb.push('></iframe>');
...
function closeShadeDiv(){
	...
	if(loadProceedImg!=undefined)loadProceedImg.removeNode(true);
	...
	if(deleteIframe!=undefined)deleteIframe.removeNode(true);
